...
export class MarketBaselineService {
  ...
  getBaseline(product: any): MarketBaseline {
  ...
  getBaselines(products: any[]): MarketBaseline[] {
    return products.map(p => this.getBaseline(p));
  }

  private isAntibiotic(product: any): boolean {
  ...
  private isAntiInflammatory(product: any): boolean {
    const text = `${product.category || ''}`.toLowerCase();
  ...
  private isChronic(product: any): boolean {
    const text = `${product.category || ''}`.toLowerCase();
  ...
  private isDermo(product: any): boolean {
    const text = `${product.category || ''}`.toLowerCase();
