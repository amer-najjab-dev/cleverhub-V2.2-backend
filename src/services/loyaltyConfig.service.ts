...
export class LoyaltyConfigService {
  ...
  async getActiveConfig(): Promise<LoyaltyConfig> {
  ...
  private getCachedConfigSync(): LoyaltyConfig {
  ...
  async assignWelcomePoints(clientId: number): Promise<void> {
    const config = await this.getActiveConfig();
  ...
  simulatePoints(amount: number, isFirstPurchase: boolean = false, isBirthday: boolean = false): {
    basePoints: number;
    finalPoints: number;
