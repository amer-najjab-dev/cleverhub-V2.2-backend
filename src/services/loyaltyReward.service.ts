...
export class LoyaltyRewardService {
  private rewardRepo = AppDataSource.getRepository(LoyaltyReward);
  private packRepo = AppDataSource.getRepository(LoyaltyPack);
  private transactionRepo = AppDataSource.getRepository(LoyaltyTransaction);
  ...
  async getAllRewards(activeOnly: boolean = true): Promise<LoyaltyReward[]> {
    const where: any = {};
    if (activeOnly) {
      where.isActive = true;
      const now = new Date();
  ...
  async updateReward(id: number, data: Partial<LoyaltyReward>): Promise<LoyaltyReward> {
  ...
  async getAllPacks(activeOnly: boolean = true): Promise<LoyaltyPack[]> {
    const where: any = {};
    if (activeOnly) {
      where.isActive = true;
      const now = new Date();
  ...
  async redeemReward(clientId: number, rewardId: number, saleId?: number): Promise<{
    success: boolean;
  ...
  async redeemPack(clientId: number, packId: number, saleId?: number): Promise<{
    success: boolean;
    transaction: LoyaltyTransaction;
    newBalance: number;
  }> {
    ...
    if (!pack) throw new Error('Pack not available');
  ...
  async getLoyaltyClosure(date: Date): Promise<{
    realSales: number;
  ...
  async getPromotionSuggestions(): Promise<any[]> {
  ...
  async getWeeklyStrategy(): Promise<any> {
  ...
  private calculateTier(points: number): 'Bronze' | 'Argent' | 'Or' {
    if (points >= 5000) return 'Or';
    if (points >= 2000) return 'Argent';
    return 'Bronze';
  }
}
