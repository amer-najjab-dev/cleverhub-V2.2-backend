...
export class CampaignController {
  ...
  async deleteTemplate(req: Request, res: Response) {
    try {
      ...
      // Verificar que la plantilla pertenece a la farmacia
      const template = await prisma.message_templates.findFirst({
        where: {
          id: parseInt(id),
          pharmacy_id: pharmacyId,
        },
  ...
  async sendCampaign(req: Request, res: Response) {
    try {
      ...
      const campaign = await prisma.message_campaigns.findFirst({
        where: { 
          id: parseInt(id),
          pharmacy_id: pharmacyId,
        },
      ...
      for (const recipient of campaign.recipients) {
        try {
          ...
          await prisma.message_recipients.update({
      ...
      const updatedCampaign = await prisma.message_campaigns.update({
        where: { id: parseInt(id) },
        data: { 
          status: 'sent', 
          sent_at: new Date(),
  ...
  async getCampaignRecipients(req: Request, res: Response) {
    try {
      ...
      const where: any = { campaign_id: parseInt(id) };
      if (status) {
        where.status = status as string;
      }
      
      const recipients = await prisma.message_recipients.findMany({
        where,
        include: {
          client: {
            select: {
      ...
      const total = await prisma.message_recipients.count({ where });
  ...
  async deleteCampaign(req: Request, res: Response) {
    try {
      ...
      await prisma.message_campaigns.delete({
        where: { id: parseInt(id) },
      });
