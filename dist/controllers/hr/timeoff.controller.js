...
exports.timeOffController = {
    getAll: async (req, res) => {
        try {
            ...
            const requests = await server_1.prisma.time_off_requests.findMany({
                where,
            ...
            const requestsWithDetails = await Promise.all(requests.map(async (req) => {
                ...
                let reviewer = null;
                if (req.reviewed_by) {
                    reviewer = await server_1.prisma.users.findUnique({
    ...
    create: async (req, res) => {
        try {
            ...
            const request = await server_1.prisma.time_off_requests.create({
                data: {
