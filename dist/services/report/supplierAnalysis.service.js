...
class SupplierAnalysisService {
    async getSupplierAnalysis(filters) {
        ...
        const analysis = await Promise.all(suppliers.map(async (supplier) => {
            // Compras del período
            const purchases = await server_1.prisma.product_purchases.findMany({
                where: {
            ...
            const creditNotes = await server_1.prisma.credit_notes.findMany({
                where: {
                    supplier_id: supplier.id,
                    issue_date: {
                        gte: startDate,
            ...
            // Totales
            const totalPurchases = purchases.reduce((sum, p) => sum + (Number(p.price_pph) * p.quantity), 0);
            const totalCreditNotes = creditNotes.reduce((sum, c) => sum + Number(c.total_amount), 0);
